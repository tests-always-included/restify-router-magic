...
module.exports = function (findMethod, path, requireFn) {
    ...
    function filenameToPaths(config, filename) {
        ...
        if (uriPath.substr(-(config.indexName.length + 1)) === "/" + config.indexName) {
            uriPath = uriPath.substr(0, uriPath.length - config.indexName.length);
        }
    ...
    function moduleLoader(server, config, filename, uri, uniqueNames) {
        ...
        // TypeScript export default factory function
        if (typeof module === "object" && typeof module.default === "function") {
            module = module.default(server, uri, config.options);
        }
